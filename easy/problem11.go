...
func greatestProductInGrid(
	...
	for r := 0; r < len(grid); r++ {
		for c := 0; c < len(grid[0]); c++ {
			p := greatestProductFromPoint(grid, n, r, c)
			if p > maxSeen {
				maxSeen = p
			}
		}
...
func greatestProductFromPoint(
	...
	// down+right diagonal
	if r+n < len(grid) && c+n < len(grid[r]) {
		for i := 0; i < n; i++ {
			downright *= grid[r+i][c+i]
		}
	}

	// down+left diagonal
	if r+n < len(grid) && c-n >= 0 {
		for i := 0; i < n; i++ {
			downleft *= grid[r+i][c-i]
		}
	}
